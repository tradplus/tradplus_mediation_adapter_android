 ...
 * implementing subclasses of CustomEventInterstitial, you can enable the TradPlus SDK to natively
 * support a wider variety of third-party ad networks, or execute any of your application code on
 * demand.
 * 
 * At runtime, the TradPlus SDK will find and instantiate a CustomEventInterstitial subclass as needed
...
public abstract class CPCustomEventInterstitial extends BaseTimeOutAdapter {
    ...
    public CustomEventBidingCallback getCustomEventBidingCallback() {
        return customEventBidingCallback;
    }

    /*
     * When the TradPlus SDK receives a response indicating it should load a custom event, it will send
     ...
     * your application by calling TradPlusInterstitial.setLocalExtras(Map<String, Object>). Note that
     ...
     * TradPlus website that you want to associate with a given custom event request. This data may be
    ...
    public interface CustomEventInterstitialListener {
         ...
         * stall.
         */
        void onInterstitialFailed(TPError error);
        ...
        void onInterstitialRewarded(String currencyName, int amount);
