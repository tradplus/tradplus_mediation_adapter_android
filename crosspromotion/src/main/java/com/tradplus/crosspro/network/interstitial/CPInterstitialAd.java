...
import static com.tradplus.crosspro.common.CPConst.FORMAT.INTERSTITIAL_FORMAT;
import static com.tradplus.crosspro.network.base.CPErrorCode.exception;
import static com.tradplus.crosspro.network.base.CPErrorCode.fail_null_context;
...
public class CPInterstitialAd extends CPBaseAd {
    ...
    public CPInterstitialAd(Context context, String campaignId, String adSourceId, int full_screen) {
    ...
    public void load() {
        ...
        cpAdConfigController.loadConfig(getContext(), campaignId, adSourceId, INTERSTITIAL_FORMAT, 0, direction);
    ...
    public void show() {
        ...
        try {
            ...
            if (mContext == null || _cpAdResponse == null) {
                if (cpRewardVideoAdListener != null) {
                    cpRewardVideoAdListener.onVideoShowFailed(CPErrorCode.get(noADError, fail_null_context));
            ...
            if (full_screen != 1) {
                DisplayMetrics dm = GlobalTradPlus.getInstance().getContext().getResources().getDisplayMetrics();
                Context context = GlobalTradPlus.getInstance().getActivity();
                if (context != null) {
                    HalfScreenDialog dialog = new HalfScreenDialog(context);
                    dialog.start(_cpAdResponse, orientation ? SCREEN_LAND_TYPE : SCREEN_PORT_TYPE, timeStamp, adSourceId, true, dm.widthPixels, dm.heightPixels, direction);
                    dialog.setCanceledOnTouchOutside(false);
                    dialog.show();
                } else {
                    if (cpRewardVideoAdListener != null) {
                        cpRewardVideoAdListener.onVideoShowFailed(new CPError(exception, fail_null_context));
                    }
                }
            } else {
                CPAdActivity.start(getContext(), _cpAdResponse, orientation ? SCREEN_LAND_TYPE : SCREEN_PORT_TYPE, timeStamp, adSourceId, 1, true, direction);
            }
        } catch (Exception e) {
            e.printStackTrace();
            if (cpRewardVideoAdListener != null) {
